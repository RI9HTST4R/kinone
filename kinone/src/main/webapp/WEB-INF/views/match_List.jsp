...
<form id="list-condition">
	...
	<input type="hidden" id="matchlist-seasoncode" name="seasoncode" value="${seasonList[0]}"/>
	<input type="hidden" id="matchlist-month" name="month" value="${nowMonth}"/>
</form>
<input type="hidden" id="topseason" value="${seasonList[0]}"/>
<script>
	var today = new Date();
	...
	// 시즌 변경
	function changeSeason(seasoncode){
	//	alert(seasoncode);
		$("#matchlist-seasoncode").val(seasoncode);
		getMatchList();
	}
			...
			$.each(dayarr, function(idx, value){
				...
				$.each(matcharr, function(idx2, value2){
					...
					if(day == day2){
						...
						if(value2.mstatus == 0){
							content += '<button class="btn btn-warning" type="button">전력비교 &gt;</button>';
							...
							content += '<button class="btn btn-danger" type="button">경기결과 &gt;</button>';
						}
						content += '</td></tr></table>';
					}
